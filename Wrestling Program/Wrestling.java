...
public class Wrestling{
   public static void main(String[] args) {
      int howManyTeams = 2;
      int howManyWrestlers = 3;

      int[] teamWins = new int[howManyTeams];
      Teams[] teamsArray = new Teams[howManyTeams];
      Teams team = new Teams();
      Tournament tournament = new Tournament(howManyTeams);
      ...
      /*for(int i = 0; i < teamsArray.length; i++){
         ...
         teamsArray[i].initializeTeams(usedNameIndexes, namePool, howManyWrestlers, teamWins);
      }//End of for loop*/

      team.initializeTeams(usedNameIndexes, namePool, howManyWrestlers, teamWins, teamsArray);
      
      tournament.outputTeamsAndTeamAtributes(teamsArray, teamWins, new int[howManyTeams], 0);
   ...
   static class Wrestlers{
      ...
      Wrestlers(int id, int weight, double  str, int team, int wrestlerWins, String name) {
         this.id = id;
         this.weight = weight;
         this.str = str;
         this.team = team;
         this.name = name;
         this.wrestlerWins = wrestlerWins;
   ...
   static class Teams {
      ...
      void initializeTeams(ArrayList<Integer> usedNameIndexes, String[] namePool, int howManyWrestlers, int[] teamWins, Teams[] teamsArray) {
         Random random = new Random();

         for (int j = 0; j < teamsArray.length; j++) {
            teamsArray[j] = new Teams();
            teamsArray[j].wrestlers = new Wrestlers[howManyWrestlers];

            for (int i = 0; i < teamsArray[j].wrestlers.length; i++) {
               int id = random.nextInt(101); 
               int weight = random.nextInt(81) + 70; // Range: 50 to 150
               double str = 1 + (9 * random.nextDouble()); // Range: 1.0 to 10.0
               int wrestlerWins = 0; // Set wrestlerWins to 8

               teamsArray[j].wrestlers[i] = new Wrestlers(
                  id, 
                  weight,
                  str,
                  j,
                  wrestlerWins,
                  Wrestlers.addWrestlerNames(usedNameIndexes, namePool)
               );
            }
         }
      }
   ...
   static class Tournament{
      ...
      Tournament(int howManyTeams) {
         this.howManyTeams = howManyTeams;
         this.teamWins = new int[howManyTeams];
      }//End of Contructor

      void outputTeamsAndTeamAtributes(Teams[] teamsArray, int[] teamWins, int[] wins, int team) {
         for(int i = 0; i < teamsArray.length; i++){
         ...
         for (int i = 0; i < teamsArray.length; i++) {
               System.out.println("Team " + (i + 1) + ":" + " - Score: " + teamWins[i]);
            for (int j = 0; j < teamsArray[i].wrestlers.length; j++) {
               Wrestlers wrestler = teamsArray[i].wrestlers[j];
               String[] parts = wrestler.name.split(" the ");
               String characteristic = (parts.length == 2) ? parts[1] : "No characteristic provided";
               System.out.println("Name: " + wrestler.name + ", Team: "+ team + ", Characteristic: " + characteristic + ", Strength: " + wrestler.str + ", Weight: " + wrestler.weight + ", ID: " + wrestler.id + ", Wins: " + wrestler.wrestlerWins);
               System.out.println();
            }
               System.out.println();
         }
